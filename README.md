...
**How the checker gets verified.** The checker is written in Rust and translated to **Lean 4 via [Aeneas](https://github.com/AeneasVerif/aeneas)**, where its soundness theorem (*accept ⇒ UNSAT*) is discharged — the "not lots of hand-math" path. This is the chosen route; the alternative of writing the checker directly in Lean (the `bv_decide` style) is kept as a documented fallback. The Lean proofs build with `elan` + `lake` in the required Lean CI job, which regenerates the Aeneas model from the Rust source before every proof build (`rules_lean` is only a commented placeholder in `MODULE.bazel`).
...
- **Bitvector ops:** `bvadd`, `bvsub`, `bvmul`, `bvudiv`, `bvurem`, `bvand`, `bvor`, `bvxor`, `bvshl`, `bvlshr`, `bvashr`, `bvrotr`, `extract{hi,lo}`, `concat`, `zero_ext{by}`, `sign_ext{by}`, plus `ite` (the bool→BV bridge).
...
The **sliver** — non-extensional `Array(BV32 → BV8)` select/store (concrete **and symbolic** indices) plus uninterpreted `pure_call` congruence — is implemented as a separate, preprocessed-away layer (`Solver::check_sliver`, `src/sliver.rs`); it is eliminated into the closed fragment before bit-blasting, so no array/UF op ever reaches the AIG.
...
crate, `rules_wasm_component` for the wasip2 component; the Lean proofs are
not built by Bazel — they build with `elan` + `lake` in CI, and `rules_lean`
is only a commented `MODULE.bazel` placeholder):
...
The roadmap lives in the rivet graph (`artifacts/features.yaml`,
FEAT-000…FEAT-015) — see [ROADMAP.md](ROADMAP.md) for how to query it. All
phases P0–P5 are verified and shipped (releases v0.1.0–v0.19.0); current
work is planned per release on the GitHub milestones. The phases were:
